...
{{-- LAYOUT --}}
{{-- ========================================================= --}}
{{-- Tambahkan state mobileMenuOpen pada parent utama --}}
            ...
            <div x-show="mobileMenuOpen"
                 ...
                 <div class="flex h-20 shrink-0 items-center border-b border-gray-100 px-6">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Kelola Tugas Tanpa Ribet!</p>
                    </div>
                 </div>
            ...
            destroy() {
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-700 p-6 text-white shadow-sm md:p-8">
                    <div class="relative z-10">
                        ...
                        <h1 class="mt-2 text-3xl font-bold md:text-4xl">Halo, {{ Auth::user()->name }}!</h1>
                        <p class="mt-3 max-w-2xl text-sm text-indigo-100 md:text-base">Kelola tugas sekolah, proyek, dan aktivitas pribadi dengan lebih teratur.</p>
                    </div>
                ...
                {{-- ================================================= --}}
                @if ($dueSoonCount > 0)
                    <div class="mt-6 flex items-center gap-3 rounded-2xl border border-rose-100 bg-rose-50 px-5 py-4 text-rose-700">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-100">⚠️</div>
                        <div>
                            <p class="font-semibold">Ada tugas yang mendekati deadline!</p>
                            <p class="text-sm text-rose-600">{{ $dueSoonCount }} tugas memiliki deadline dalam 7 hari ke depan.</p>
                        </div>
                    </div>
                @else
                    <div class="mt-6 flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 px-5 py-4 text-emerald-600">
                        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100">✓</div>
                        <div>
                            <p class="font-semibold">Semua aman</p>
                            <p class="text-sm text-emerald-700">Tidak ada deadline dalam 7 hari ke depan.</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-6 grid grid-cols-1 gap-5 md:grid-cols-3">
                    {{-- Total --}}
                    <div class="group rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-indigo-100 hover:shadow-lg">
                        <p class="text-sm font-medium text-gray-500 transition group-hover:text-indigo-600">Total Tugas</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900 transition group-hover:text-indigo-600">{{ $totalTasks }}</p>
                        <p class="mt-2 text-xs text-gray-400">Semua tugasmu</p>
                    </div>

                    {{-- Completed --}}
                    <div class="group rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-100 hover:shadow-lg">
                        <p class="text-sm font-medium text-gray-500 transition group-hover:text-emerald-600">Sudah Selesai</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900 transition group-hover:text-emerald-600">{{ $completedTasks }}</p>
                        <p class="mt-2 text-xs text-gray-400">Tugas yang selesai</p>
                    </div>

                    {{-- Deadline --}}
                    <div class="group rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-rose-100 hover:shadow-lg">
                        <p class="text-sm font-medium text-gray-500 transition group-hover:text-rose-600">Deadline H-7</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900 transition group-hover:text-rose-600">{{ $dueSoonCount }}</p>
                        <p class="mt-2 text-xs text-gray-400">Perlu segera dikerjakan</p>
                    </div>
                </div>
                ...
                @if ($priorityTasks->count() > 0)
                    <div class="mt-6 overflow-hidden rounded-2xl border border-rose-100 bg-white shadow-sm">
                        <div class="border-b border-rose-100 bg-rose-50 px-6 py-5">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="font-semibold text-gray-900">Prioritas Deadline</h3>
                                    <p class="text-xs text-gray-500">Tugas dengan deadline maksimal 7 hari ke depan</p>
                                </div>
                            </div>
                        </div>
                        ...
                        @foreach ($priorityTasks as $task)
                            ...
                            <div class="flex items-center gap-4 border-b border-gray-50 px-6 py-4 last:border-0 hover:bg-gray-50">
                                <div class="min-w-0 flex-1">
                                    <a href="{{ route('tasks.show', $task) }}" class="block truncate font-medium text-gray-900 hover:text-indigo-600">
                                        {{ $task->judul }}
                                    </a>
                                    <p class="mt-1 text-xs text-gray-400">Deadline: {{ $deadline->translatedFormat('d F Y') }}</p>
                                </div>

                                @if ($daysLeft == 0)
                                    <span class="rounded-full bg-rose-100 px-3 py-1 text-xs font-semibold text-rose-700">Hari ini</span>
                                @elseif ($daysLeft == 1)
                                    <span class="rounded-full bg-orange-100 px-3 py-1 text-xs font-semibold text-orange-700">Besok</span>
                                @else
                                    <span class="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">H-{{ $daysLeft }}</span>
                                @endif

                                <a href="{{ route('tasks.show', $task) }}" class="text-xs font-medium text-indigo-600 hover:text-indigo-800">Detail</a>
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="mt-6 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                    ...
                    <div class="border-b border-gray-100 px-6 py-5">
                        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                            {{-- TITLE --}}
                            <div class="flex items-center gap-3">
                                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600">📋</div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if (!$search && $category === 'all')
                                            <span class="rounded-full bg-gray-100 px-2 py-1 text-xs font-medium text-gray-500">{{ $totalTasks }}</span>
                                        @endif
                                    </div>
                                ...
                                <div class="flex w-full flex-col gap-2 sm:flex-row">
                                    ...
                                    <form action="{{ route('dashboard') }}" method="GET" class="flex min-w-0 flex-1">
                                        ...
                                        <div class="relative w-full">
                                            <input type="text" name="search" value="{{ $search }}" placeholder="Cari task..." class="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-100">
                                        ...
                                        <button type="submit" class="ml-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700">Cari</button>
                                    </form>
                                    ...
                                    <form action="{{ route('dashboard') }}" method="GET">
                                        ...
                                        <select name="sort" onchange="this.form.submit()" class="w-full rounded-xl border border-gray-200 bg-white pl-4 pr-10 py-2.5 text-sm font-medium text-gray-600 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 sm:w-auto">
                                    ...
                                    <form action="{{ route('dashboard') }}" method="GET">
                                        ...
                                        <select name="category" onchange="this.form.submit()" class="w-full rounded-xl border border-gray-200 bg-white pl-4 pr-10 py-2.5 text-sm font-medium text-gray-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 sm:w-auto">
                                    ...
                                    {{-- RESET --}}
                                    @if ($search || $category !== 'all')
                                        <a href="{{ route('dashboard', ['sort' => $sort]) }}" class="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-center text-sm font-medium text-gray-600 transition hover:bg-gray-50 hover:text-rose-600">Reset</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    @forelse ($tasks as $task)
                        ...
                        <div class="group flex items-center gap-4 border-b border-gray-50 px-6 py-4 last:border-0 transition hover:bg-gray-50/70">
                            <span class="w-1.5 self-stretch rounded-full bg-indigo-500"></span>

                            <div class="min-w-0 flex-1">
                                <a href="{{ route('tasks.show', $task) }}" class="block truncate font-medium transition {{ $isDone ? 'text-gray-400 line-through' : 'text-gray-900 hover:text-indigo-600' }}">
                                    {{ $task->judul }}
                                </a>
                            ...
                            <div class="flex shrink-0 items-center gap-3">
                                @if ($task->priority)
                                    ...
                                    @endphp
                                    <span class="hidden rounded-full px-2.5 py-1 text-xs font-medium sm:inline-flex {{ $priorityClass }}">{{ $task->priority }}</span>
                                @endif

                                @if ($task->deadline)
                                    <span class="hidden text-xs text-gray-400 md:inline">{{ \Carbon\Carbon::parse($task->deadline)->translatedFormat('d M') }}</span>
                                @else
                                    <span class="hidden text-xs text-gray-400 md:inline">Tanpa deadline</span>
                                @endif

                                @if ($isDone)
                                    <span class="hidden rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 lg:inline-flex">Selesai</span>
                                @endif

                                <a href="{{ route('tasks.show', $task) }}" class="text-xs font-medium text-indigo-600 transition hover:text-indigo-800">Detail</a>
                            </div>
                        </div>

                    @empty
                        <div class="px-6 py-16 text-center">
                            <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-gray-100 text-gray-400">🔎</div>
                            ...
                            @if ($search || $category !== 'all')
                                <a href="{{ route('dashboard', ['sort' => $sort]) }}" class="mt-4 inline-flex rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700">Reset Filter</a>
                            @else
                                <a href="{{ route('tasks.create') }}" class="mt-4 inline-flex rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700">+ Tambah Tugas</a>
                            @endif
                        </div>
                    @endforelse
                    ...
                    @if (!$search && $category === 'all' && $totalTasks > 5)
                        <div class="border-t border-gray-100 px-6 py-4 text-center">
                            <a href="{{ route('tasks.index') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">Lihat semua {{ $totalTasks }} tugas →</a>
                        </div>
                    @endif
                </div>
                ...
                {{-- TIPS --}}
                {{-- ================================================= --}}
                <div class="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50 px-6 py-4">
                    <p class="text-sm font-medium text-indigo-900">💡 Tips mengelola tugas</p>
                    <p class="mt-1 text-sm text-indigo-700">Gunakan prioritas, kategori, dan deadline untuk menentukan tugas mana yang harus dikerjakan terlebih dahulu.</p>
                </div>
